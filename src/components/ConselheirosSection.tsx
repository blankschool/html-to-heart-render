    ...
    <div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: '1px', background: 'rgba(240,237,230,0.08)', border: '1px solid rgba(240,237,230,0.08)' }}>
      {conselheiros.map((c, i) => (
        <div key={i} className="bg-ink p-[40px_32px] transition-colors hover:bg-ink-2">
          ...
          <div className="font-serif-body text-[22px] font-normal text-cream mb-1">{c.name}</div>
          <div className="font-mono-brand text-[12px] leading-[1.5] tracking-[0.3px] mb-[18px]" style={{ color: 'rgba(240,237,230,0.78)' }}>{c.role}</div>
          <p className="text-[15px] font-light leading-[1.8] whitespace-pre-line" style={{ color: 'rgba(240,237,230,0.90)' }}>{c.bio}</p>
          <div className="mt-[18px] pt-3.5 font-mono-brand text-[11px] tracking-[0.5px]" style={{ borderTop: '1px solid rgba(240,237,230,0.08)', color: 'rgba(240,237,230,0.65)' }}>
