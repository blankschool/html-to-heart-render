...
const QuemSomosSection = () => (
  <section className="py-[72px] px-6 md:py-[100px] md:px-[52px] bg-ink" style={{ borderBottom: '1px solid rgba(240,237,230,0.08)' }}>
    ...
    <h2 className="font-display font-normal leading-[0.96] text-cream mb-6" style={{ fontSize: 'clamp(32px, 4vw, 58px)', letterSpacing: '-2px' }}>
      A maior referência em marca pessoal<br />e geração de receita através<br /><em className="italic" style={{ color: 'rgba(240,237,230,0.3)' }}>das redes sociais.</em>
    </h2>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 mb-14" style={{ gap: '1px', background: 'rgba(240,237,230,0.08)', border: '1px solid rgba(240,237,230,0.08)' }}>
      {stats.map((s, i) => (
        <div key={i} className="bg-ink p-[40px_36px_32px] relative overflow-hidden" style={{ borderTop: i === 0 ? '2px solid rgba(240,237,230,0.45)' : undefined }}>
          ...
          <div className="font-mono-brand text-[10px] tracking-[1.5px] uppercase" style={{ color: 'rgba(240,237,230,0.6)' }}>{s.label}</div>
          <div className="text-[12px] font-light leading-[1.65] mt-3.5 pt-3.5" style={{ color: 'rgba(240,237,230,0.65)', borderTop: '1px solid rgba(240,237,230,0.04)' }}>{s.desc}</div>
        </div>
      ))}
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: '1px', background: 'rgba(240,237,230,0.08)', border: '1px solid rgba(240,237,230,0.08)', borderTop: 'none' }}>
      <div className="bg-ink p-[40px_36px]">
        <div className="font-mono-brand text-[9px] tracking-[3px] uppercase mb-4" style={{ color: 'rgba(240,237,230,0.5)' }}>Origem</div>
        <p className="font-serif-body text-[15px] font-normal leading-[1.75]" style={{ color: 'rgba(240,237,230,0.82)' }}>
      ...
      <div className="bg-ink p-[40px_36px]">
        <div className="font-mono-brand text-[9px] tracking-[3px] uppercase mb-5" style={{ color: 'rgba(240,237,230,0.5)' }}>Portfólio</div>
        <div className="grid grid-cols-2 gap-0">
          {portfolio.map((p, i) => (
            <div key={i} className="py-2.5" style={{ borderBottom: i < portfolio.length - 2 ? '1px solid rgba(240,237,230,0.04)' : 'none' }}>
              <div className="text-[13px] font-normal" style={{ color: 'rgba(240,237,230,0.82)' }}>{p.name}</div>
              {p.co && <div className="font-mono-brand text-[9px] mt-0.5" style={{ color: 'rgba(240,237,230,0.4)' }}>{p.co}</div>}
            </div>
          ))}
        </div>
      </div>
