  ...
  <section className="py-[72px] px-6 md:py-[100px] md:px-[52px] bg-ink" style={{ borderBottom: '1px solid rgba(240,237,230,0.08)' }}>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 mb-14" style={{ gap: '1px', background: 'rgba(240,237,230,0.08)', border: '1px solid rgba(240,237,230,0.08)' }}>
      {stats.map((s, i) => (
        <div key={i} className="bg-ink p-[40px_36px_32px] relative overflow-hidden" style={{ borderTop: i === 0 ? '2px solid rgba(240,237,230,0.45)' : undefined }}>
          ...
          <div className="font-mono-brand text-[12px] tracking-[1.5px] uppercase" style={{ color: 'rgba(240,237,230,0.6)' }}>{s.label}</div>
          <div className="text-[14px] font-light leading-[1.65] mt-3.5 pt-3.5" style={{ color: 'rgba(240,237,230,0.85)', borderTop: '1px solid rgba(240,237,230,0.04)' }}>{s.desc}</div>
        </div>
      ))}
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: '1px', background: 'rgba(240,237,230,0.08)', border: '1px solid rgba(240,237,230,0.08)', borderTop: 'none' }}>
      <div className="bg-ink p-[40px_36px]">
        <div className="font-mono-brand text-[11px] tracking-[3px] uppercase mb-4" style={{ color: 'rgba(240,237,230,0.72)' }}>Origem</div>
        <p className="font-serif-body text-[17px] font-normal leading-[1.75]" style={{ color: 'rgba(240,237,230,0.92)' }}>
      ...
      <div className="bg-ink p-[40px_36px]">
        <div className="font-mono-brand text-[11px] tracking-[3px] uppercase mb-5" style={{ color: 'rgba(240,237,230,0.72)' }}>Portfólio</div>
        <div className="grid grid-cols-2 gap-0">
          {portfolio.map((p, i) => (
            <div key={i} className="py-2.5" style={{ borderBottom: i < portfolio.length - 2 ? '1px solid rgba(240,237,230,0.04)' : 'none' }}>
              <div className="text-[15px] font-normal" style={{ color: 'rgba(240,237,230,0.92)' }}>{p.name}</div>
              {p.co && <div className="font-mono-brand text-[11px] mt-0.5" style={{ color: 'rgba(240,237,230,0.65)' }}>{p.co}</div>}
            </div>
          ))}
        </div>
      </div>
