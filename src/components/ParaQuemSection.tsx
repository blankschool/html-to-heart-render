  ...
  <section className="py-[72px] px-6 md:py-[100px] md:px-[52px] bg-ink" style={{ borderBottom: '1px solid rgba(240,237,230,0.08)' }}>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 items-start">
      <div>
        ...
        <p className="text-[17px] font-light leading-[1.75]" style={{ color: 'rgba(240,237,230,0.92)' }}>
      ...
      <div>
        <div className="font-mono-brand text-[11px] tracking-[3px] uppercase mb-5" style={{ color: 'rgba(240,237,230,0.68)' }}>Perfil</div>
        <div className="flex flex-col" style={{ border: '1px solid rgba(240,237,230,0.08)' }}>
          {specs.map(([k, v], i) => (
            <div key={i} className="flex justify-between items-baseline gap-4 py-[15px] px-5 text-[15px]" style={{ borderBottom: i < specs.length - 1 ? '1px solid rgba(240,237,230,0.08)' : 'none' }}>
              <span className="font-light" style={{ color: 'rgba(240,237,230,0.78)' }}>{k}</span>
              <span className="font-medium text-cream text-right font-mono-brand text-[14px]">{v}</span>
            </div>
          ))}
        </div>
      </div>
