...
const whenColors: Record<Track, string> = {
  ind: 'rgba(240,237,230,0.45)',
  col: 'rgba(240,237,230,0.3)',
  adv: 'rgba(212,200,154,0.4)',
};
  ...
  <section id="estrutura" className="py-[72px] px-6 md:py-[100px] md:px-[52px] bg-ink-2" style={{ borderBottom: '1px solid rgba(240,237,230,0.08)' }}>
    ...
    <div className="flex gap-6 items-center mb-8 flex-wrap">
      {[
        ...
        <div key={i} className="flex items-center gap-2 font-mono-brand text-[12px] tracking-[0.5px]" style={{ color: 'rgba(240,237,230,0.78)' }}>
    ...
    <div className="grid grid-cols-1 md:grid-cols-4" style={{ gap: '1px', background: 'rgba(240,237,230,0.08)', border: '1px solid rgba(240,237,230,0.08)' }}>
      {pilares.map((p, i) => (
        <div key={i} className="bg-ink p-[24px_20px_28px] relative transition-colors hover:bg-ink-2">
          ...
          <span className="font-mono-brand text-[12px] block mb-3.5" style={{ color: 'rgba(240,237,230,0.55)' }}>{p.num}</span>
          <div className="text-[15px] font-medium mb-[7px] leading-[1.3]" style={{ color: 'rgba(240,237,230,0.9)' }}>{p.title}</div>
          <p className="text-[14px] font-light leading-[1.6]" style={{ color: 'rgba(240,237,230,0.72)' }}>{p.body}</p>
          <div className="mt-4 font-mono-brand text-[11px] tracking-[1px]" style={{ color: whenColors[p.track] }}>{p.when}</div>
        </div>
      ))}
    </div>
  </section>
