...
const RoteirosSection = () => (
  <section className="py-[72px] px-6 md:py-[100px] md:px-[52px] bg-ink" style={{ borderBottom: '1px solid rgba(240,237,230,0.08)' }}>
    ...
    <p className="text-[15px] font-light leading-[1.85] max-w-[560px]" style={{ color: 'rgba(240,237,230,0.82)' }}>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 mt-12" style={{ gap: '1px', background: 'rgba(240,237,230,0.08)', border: '1px solid rgba(240,237,230,0.08)' }}>
      {roteiros.map((r, i) => (
        <div key={i} className="bg-ink p-[32px_28px] transition-colors hover:bg-ink-2">
          <div className="font-mono-brand text-[9px] mb-2.5 tracking-[1px]" style={{ color: 'rgba(240,237,230,0.25)' }}>{r.num}</div>
          <div className="font-serif-body text-[20px] font-normal mb-[18px]" style={{ color: 'rgba(240,237,230,0.9)' }}>{r.title}</div>
          {r.models.map((m, j) => (
            <div key={j} className="flex gap-2.5 items-baseline py-2.5 text-[12.5px] font-light" style={{ color: 'rgba(240,237,230,0.78)' , borderBottom: j < r.models.length - 1 ? '1px solid rgba(240,237,230,0.04)' : 'none' }}>
              <span className="font-mono-brand text-[10px] flex-shrink-0" style={{ color: 'rgba(240,237,230,0.4)' }}>{String.fromCharCode(65 + j)}</span>
              {m}
            </div>
          ))}
        </div>
    ...
    <div className="grid grid-cols-3" style={{ gap: '1px', background: 'rgba(240,237,230,0.08)', border: '1px solid rgba(240,237,230,0.08)', borderTop: 'none' }}>
      <div className="bg-ink-2 p-[20px_24px] flex flex-col items-center justify-center gap-1">
        ...
        <div className="font-mono-brand text-[9px] tracking-[1px] text-center" style={{ color: 'rgba(240,237,230,0.55)' }}>linhas editoriais</div>
      </div>
      <div className="bg-ink-3 p-[20px_24px] flex flex-col items-center justify-center gap-1">
        <div className="font-mono-brand text-[24px]" style={{ color: 'rgba(240,237,230,0.25)' }}>× 2</div>
        <div className="font-mono-brand text-[9px] tracking-[1px] text-center" style={{ color: 'rgba(240,237,230,0.55)' }}>modelos por linha</div>
      </div>
      <div className="bg-ink-2 p-[20px_24px] flex flex-col items-center justify-center gap-1">
        <div className="font-display text-[36px] font-normal text-cream leading-none" style={{ letterSpacing: '-1px' }}>8</div>
        <div className="font-mono-brand text-[9px] tracking-[1px] text-center" style={{ color: 'rgba(240,237,230,0.55)' }}>roteiros entregues</div>
      </div>
    </div>
    <div className="mt-4 font-mono-brand text-[10px] text-center" style={{ color: 'rgba(240,237,230,0.4)' }}>
