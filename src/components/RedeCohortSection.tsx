    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 items-end mb-12">
      <div>
        ...
        <h2 className="font-display font-normal leading-[0.96] text-cream" style={{ fontSize: 'clamp(40px, 5vw, 72px)', letterSpacing: '-2px' }}>
          Nenhum creator<br />cresceu sozinho.<br /><em className="italic" style={{ color: 'rgba(240,237,230,0.3)' }}>Todos usaram</em><br /><em className="italic" style={{ color: 'rgba(240,237,230,0.3)' }}>uma rede de collabs.</em>
        </h2>
      </div>
    ...
    <div className="flex flex-col mt-12" style={{ border: '1px solid rgba(240,237,230,0.08)' }}>
      {rows.map(([k, v], i) => (
        <div key={i} className="grid grid-cols-2" style={{ borderBottom: i < rows.length - 1 ? '1px solid rgba(240,237,230,0.08)' : 'none' }}>
          <div className="py-[18px] px-6 text-[13px] font-light" style={{ color: 'rgba(240,237,230,0.65)', borderRight: '1px solid rgba(240,237,230,0.08)' }}>{k}</div>
          <div className="py-[18px] px-6 text-[13px] font-medium" style={{ color: 'rgba(240,237,230,0.9)' }}>{v}</div>
        </div>
      ))}
    </div>
  </section>
