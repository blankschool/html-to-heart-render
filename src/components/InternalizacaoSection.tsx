    ...
    <div className="mt-14" style={{ borderLeft: '1px solid rgba(240,237,230,0.08)' }}>
      {timeline.map((t, i) => (
        <div key={i} className="grid grid-cols-1 md:grid-cols-[220px_1fr] gap-4 md:gap-10 py-7 pl-9 relative" style={{ borderBottom: i < timeline.length - 1 ? '1px solid rgba(240,237,230,0.04)' : 'none' }}>
          ...
          <div className="font-mono-brand text-[10px] font-medium tracking-[2px] uppercase pt-0.5" style={{ color: t.highlight ? 'rgba(240,237,230,0.7)' : 'rgba(240,237,230,0.4)' }}>{t.phase}</div>
          <div>
            <div className="text-[15px] font-medium mb-1.5" style={{ color: t.highlight ? 'hsl(42 33% 92%)' : 'rgba(240,237,230,0.82)' }}>{t.title}</div>
            <p className="text-[13px] font-light leading-[1.75]" style={{ color: 'rgba(240,237,230,0.72)' }}>{t.body}</p>
          </div>
        </div>
      ))}
    </div>
